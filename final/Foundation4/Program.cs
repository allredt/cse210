...
// Scenario
// The local fitness center has hired you to write an app for their customers to track 
// their exercise. They have facilities for the following:

// Running
// Stationary Bicycles
// Swimming in the lap pool
// For each activity, they want to track the the date and the length of the activity 
// in minutes. Then, for each activity, they would like to also track the following:

// Running: distance
// Cycling: speed
// Swimming: number of laps
// For each activity, they do not want to store this information, but they would like 
// to be able to get following information (by calculation if it is not stored directly):
...
// Write a program that has a base Activity class and then has a derived class 
// for each of the three activities. The base class should contain any attributes 
// that are shared among all activities. Then, each derived class can define any 
...
// Finally, you should provide a GetSummary method to produce a string with all the 
// summary information.
...
// Once you have the classes in place, write a program that creates at least one activity 
// of each type. Put each of these activities in the same list. Then iterate through this 
